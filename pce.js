...
PlayeChessEngine.board.pieces.Piece = class Piece {
	...
	validate_validation(board, x_final, y_final) {
		if (board[x_final][y_final] instanceof PlayeChessEngine.board.pieces.NoPiece) {
			return true;
		}
		...
		if (x_diff == 0) {
			if (y_diff > 0) {
				for (let i = 1; i < y_diff; i++) {
					if (!(board[this.coords[0]][this.coords[1] + i] instanceof PlayeChessEngine.board.pieces.NoPiece)) {
						return false;
					}
				}
			}
			...
			if (x_diff > 0) {
				for (let i = 1; i < x_diff; i++) {
					if (!(board[this.coords[0] + i][this.coords[1]] instanceof PlayeChessEngine.board.pieces.NoPiece)) {
						return false;
					}
				}
			}
			...
			if (x_diff > 0 && y_diff > 0) {
				for (let i = 1; i < x_diff; i++) {
					if (!(board[this.coords[0] + i][this.coords[1] + i] instanceof PlayeChessEngine.board.pieces.NoPiece)) {
						return false;
					}
				}
			}
...
PlayeChessEngine.board.pieces.NoPiece = class NoPiece extends PlayeChessEngine.board.pieces.Piece {
	constructor() {
		super(PlayeChessEngine.board.pieces.piece_type.n, false, -1, -1);
	}
		...
		if (y_diff == 0 && x_diff != 0) {
			if (this.is_white) {
				if (x_diff == 1) {
					...
					if (!this.has_moved && board[x_final][y_final] instanceof PlayeChessEngine.board.pieces.NoPiece) {
						return true;
					}
				}
			}
			...
			if (this.is_white) {
				if (x_diff == 1 && y_diff == 1) {
					if (!(board[x_final][y_final] instanceof PlayeChessEngine.board.pieces.NoPiece) && !board[x_final][y_final].is_white) {
						return true;
					}
				}
...
PlayeChessEngine.board.Board = class Board {
	Board = [
		[new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece()],
		[new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece()],
		[new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece()],
		[new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece()],
		[new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece(), new PlayeChessEngine.board.pieces.NoPiece()],
	...
	print_board(moves = [], board = this.Board) {
		...
		for (let i = 0; i < 8; i++) {
			...
			for (let j = 0; j < 8; j++) {
				...
				if (skip)
					continue;
				if (board[7 - i][j] instanceof PlayeChessEngine.board.pieces.NoPiece) {
					board_str += "  ";
				}
				else {
	...
	get_moves(x, y, from_premove = false) {
		...
		for (let i = 0; i < 8; i++) {
			for (let j = 0; j < 8; j++) {
				if (this.Board[i][j] instanceof PlayeChessEngine.board.pieces.NoPiece)
					continue;
	...
	get_all_moves(brd, white, from_premove = false) {
		...
		for (let i = 0; i < 8; i++) {
			for (let j = 0; j < 8; j++) {
				if (brd[i][j] instanceof PlayeChessEngine.board.pieces.NoPiece)
					continue;
	...
	transfer_piece(start_coords, end_coords) {
		...
		this.board[start_coords[0]][start_coords[1]] = new PlayeChessEngine.board.pieces.NoPiece();
	}
	// Variant
	...
	move(move, white) {
		if (this.board[move.start_coords[0]][move.start_coords[1]] instanceof PlayeChessEngine.board.pieces.NoPiece) {
			move.is_valid = false;
			return move;
		}
		...
		if (this.get_moves(move.start_coords[0], move.start_coords[1]).includes(move)) {
			move.is_capture = !(this.board[move.end_coords[0]][move.end_coords[1]] instanceof PlayeChessEngine.board.pieces.NoPiece);
			this.transfer_piece(move.start_coords, move.end_coords);
			move.is_valid = true;
		}
	...
	insufficient_material() {
		...
		this.board.forEach(row => {
			row.forEach(piece => {
				if (piece instanceof PlayeChessEngine.board.pieces.NoPiece)
					return;
				switch (piece.type) {
	...
	can_castle(white, kingside) {
		...
		if (king instanceof PlayeChessEngine.board.pieces.NoPiece)
			return false;
		if (king.has_moved)
			return false;
		let rook = this.board[row][kingside ? 7 : 0];
		if (rook instanceof PlayeChessEngine.board.pieces.NoPiece)
			return false;
		if (rook.has_moved)
			return false;
		...
		for (let i = start; i < end; i++) {
			if (!(this.board[row][i] instanceof PlayeChessEngine.board.pieces.NoPiece) || this.get_all_landing_moves(!white).includes([row, i]))
				return false;
		}
		return true;
	}
	...
	get_promotion(white) {
		let row = white ? 0 : 7;
		for (let i = 0; i < 8; i++) {
			if (!(this.board[row][i] instanceof PlayeChessEngine.board.pieces.NoPiece)) {
	...
	get_en_passant_side(white, side) {
		...
		if (last_move.end_coords[1] + side < 0 || last_move.end_coords[1] + side > 7)
			return [-1, -1];
		if (!(this.board[last_move.end_coords[0]][last_move.end_coords[1] + side] instanceof PlayeChessEngine.board.pieces.NoPiece)) {
